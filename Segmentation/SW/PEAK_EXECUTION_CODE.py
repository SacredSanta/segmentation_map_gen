...
<24.08.20>
검출기를 통해 얻어온 img를 morphology 등을 통해서 peak 발견이 수월하도록 figure 을 modify 하는 코드로 보임.

input >> 검출기에서 얻어온 floodmap jpeg 파일을 사용하는 듯? 
         shape 는 (1200, 1200, 3), (1466, 1466, 3), (800, 800, 3) 등 다양함.
...
def mask_ellipse(rad):
    ...
    for theta in np.arange(0, 2 * np.pi, np.pi / prm):
        ...
        msk[int(ctr[0] + np.ceil(x)), int(ctr[1] + np.ceil(y))] = 1
...
#flood_map = cv2.imread('C:/Users/dlatj/Anaconda3/floodmap/test/MAETEL_SAMPLE.jpeg')
img_dir = "./floodmap_img/"
filename = "ori_img.jpeg"

# file이 잘 읽히는지 확인
if os.path.exists(img_dir + filename):
    flood_map = cv2.imread(img_dir + filename)
    try:
        print(flood_map.shape)
    except:
        sys.exit("cv2 가 불러오지 못합니다. (경로에 한글 있는지 확인)")    
else:
    sys.exit("파일이 존재하지 않습니다.")



# (2) RGB에서 grayscale 로 변환  --------------------------------------------------------------
img_size = np.shape(flood_map)[0]
# if image is RGB
R = flood_map[:, :, 0]
G = flood_map[:, :, 1]
B = flood_map[:, :, 2]
flood_map = (R * 299. / 1000 + G * 587. / 1000 + B * 114. / 1000)/255
...
# (1) plot part
fig, ax = plt.subplots()
fig.canvas.manager.set_window_title('PROCESS')
...
# (2) DNN model
model2 = load_model('Morphology_CNN.h5')
...
# Scanning and Peak detection
for i in width:
    for j in length:
        sub_img = flood_map[i:i + 20, j:j + 20]
        if np.mean(np.mean(sub_img)) < background_param:  # background parameter
            continue
        else:
            # input 전처리
            f = np.fft.fftshift(np.fft.fft2(sub_img))
            sz = np.shape(f)
            f_msk = mask_ellipse([2, 2]) - mask_ellipse([0.5, 0.5])
            sub_img2 = np.real(np.fft.ifft2(np.fft.ifftshift(f * f_msk)))
...
vor = Voronoi(sort_points)
fig, ax2 = plt.subplots(frameon=False)
fig.canvas.set_window_title('SEGMENTED RESULT')
ax2.imshow(flood_map)
ax2.scatter(sort_points[:, 0], sort_points[:, 1], c='red', s=3, picker=5)
ax2.set(xlim=[0., img_size], ylim=[0., img_size])
...
#regions = np.array(sorted(regions, key=itemgetter(0)))
# colorize
...
fig.canvas.set_window_title('Label RESULT')
fig.canvas.draw()
ax3.set(xlim=[0., img_size], ylim=[0., img_size])

for region in regions:
    polygon = vertices[region]
    plt.fill(*zip(*polygon), color=str(n))
    n = n+(1/len(regions))
...
for region in regions:
    ...
    mask = mask*n
    poly = poly+mask.reshape(height, width)
    n = n+1
...
position = np.where(index_map == 255)
print(sort_points)